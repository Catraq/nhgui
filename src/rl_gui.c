...
uint32_t utf8_decode(const char *s, uint32_t *codepoint)
{
...
)
{
	...
	for(uint32_t i = 0; i < *input_buffer_length; i++)
	{
		...
		float new_x_mm = x_mm + (float)(font->face->glyph->advance.x >> 6) * mm_per_pixel_x + cursor_mm;
		/* See if it is past the boudning box */
...
)
{
	...
	if(field->cursor_index < *input_buffer_length)
	{
		...
		if(FT_Load_Char(font->face, c, FT_LOAD_RENDER)){
			...
			float mm_per_pixel_x = (float)context->screen_width_mm/(float)context->screen_resolution_x;
			float cursor_width_mm = font->face->glyph->bitmap.width*mm_per_pixel_x;
